"""
實驗 001：Combined RRF (KNN + DTW + Rule-Based)

參數配置：
  - alpha (KNN 權重): 0.13
  - rule_weight (Rule-Based 權重): 0.25
  - w_dtw = 1 - alpha - rule_weight = 0.62
  - rrf_k: 60
  - k: 5
  - pool_size_factor: 10
  - impact_radius_km: 500
  - dtw_weights: [1.0, 1.0, 1.0, 0.5]
  - sakoe_chiba_ratio: 0.3

評估方式：Leave-One-Out Cross Validation (198 筆, Cat 1-9)
含降水分析

執行：python experiments/exp001_combined_rrf/run.py
"""
...
# === 實驗配置（完整記錄，供未來重現）===
EXPERIMENT_CONFIG = {
    "name": "exp001_combined_rrf",
    "description": "Combined RRF: KNN(0.13) + DTW(0.62) + Rule-Based(0.25) with rainfall analysis",
    "method": "combined",
    "parameters": {
        "alpha": 0.13,
        "rule_weight": 0.25,
...
def main():
    print("=" * 60)
    print("🧪 實驗 001: Combined RRF")
    print("=" * 60)
    print(f"  方法: {EXPERIMENT_CONFIG['method']}")
    print(
        f"  參數: alpha={EXPERIMENT_CONFIG['parameters']['alpha']}, "
        f"rule_weight={EXPERIMENT_CONFIG['parameters']['rule_weight']}, "
    ...
    meta = {
        ...
        "config_source": "experiments/exp001_combined_rrf/run.py",
